...
public class PageBean<T> {
	...
	public int getStartRow() {
		// 因为limit的参数是从0开始，所以减1，这是计算limit的第一个参数，表示所要查询数据前面的数据行数
		return (index - 1) * size;
	}
	...
	public void setNumbers(int totalPageCount) {
		if(totalPageCount>0){
			...
			for(int i = 0;i < totalPageCount;i++){
				// 保证当前页为集合的中间
				// mid = numbers.length/2+1 是要显示的页码的中间位置
				// index- (numbers.length/2+1) 如果该值小于0，说明当前页码必定是总记录中前十，这时循环页码只要大于0就可以显示
				// index- (numbers.length/2+1) 如果该值大于0，这时要求循环页码必须在mid左右范围2*(index-mid)之内，即下面两个或条件
				// 第一个或条件：
				// 第二个或条件
